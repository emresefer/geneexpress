...
%Overall, prediction error becomes almost as good as the noise of the data~($xxx$) if we use
%carefully selected $10$ points out of $40$ points. 
...
points). Figure plots median value for each time point as well as selected knots for spline
...
method to fit all of these pretty well without overfitting.
...
%We also compare the fitting performance with respect to simpler linear
%method. Average error improvement is $xxx$. Relative error is as follows:
...
Figure~\ref{fig:sup2} for mRNA clusters). After mapping each miRNA to
...
We develop \Tempselect to efficiently identify subset of important
...
\begin{figure}
\centering
\includegraphics[scale=0.2]{{../newdata/performRL2}.png}
\caption{Performance of \Tempselect by increasing number of selected points}
\label{fig:sup1}
\end{figure}
