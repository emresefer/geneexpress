...
\begin{figure}
\centering
\includegraphics[scale=0.2]{{plots/newdata/perform}.png}
...
overfitting~(See Supplementary Figure~\ref{fig:sup3} for figures of
other genes).

\begin{figure}
\begin{minipage}{1.0\textwidth}
\subfloat[PDGFRA]{\includegraphics[scale=0.12]{{plots/newdata/PDGFRA_15_all}.png}}
\hfill
...
%spline vs linear curves
We also compare the reconstruction performance of \Tempselect with fitting piecewise linear curves between initial and middle time points and between middle and
last time points. \Tempselect performs significantly better than this
reconstruction for $xxx$ genes out of $xxx$ genes~(See Supplementary
Information for $p$ value generation). We find average error
improvement to be $xxx$~(See Supplementary Figure~\ref{fig:supxxx} for
detailed plots).
...
selected mRNA points the average mean squared error is $0.3312$ whereas when
...
mRNA dataset. Spline-based reconstruction performs
better than linear reconstruction similar to mRNA dataset. Accurate
prediction of different miRNA profiles show the importance of
identified points over mRNA dataset. Among these miRNAs, mmu-miR-100 targets Fgfr3 and Igf1r,
mmu-miR-136 targets Tgfb2. Similarly, mmu-miR-152 targets Meox2,
Robo1, Fbn1, Nfya. Lastly, mmu-miR-219 targets PDGFRA, Eya2, Esr1,
Esr2, Efnb2 and Robo1 some of which are associated with BPD in preterm infants~\cite{popova2014}.
...
clusters by k-means algorithm~\cite{kmeans} where number of clusters is selected by
...
Figure~\ref{fig:sup4} for mRNA clusters). After mapping each miRNA to
corresponding genes by TargetScan~\cite{agarwal2015}, we run
...
metabolic process, etc~(See Supplementary Results for details).
...
\subsection{Comparison of Temporal Methylation Data With Expression Data}\label{sec:compareboth}
%Identified Time Points Over Temporal Methylation Data}

\Tempselect selected $0.5$, $5$, $15$, $26$ out of $8$ time points over temporal methylation data when we consider each gene locus
independently. We identified high similarity between methylation and gene expression datasets showing the
possibility of using the identified subset of time points over gene
expression also in methylation experiments. We plot both datasets jointly as in Figure~\ref{fig:methgene} for Dnmt3a, Src,
and Lox genes. Among the multiple methylation locus belonging to a gene, we select
the most similar one in terms of pearson correlation~(See Supplementary Table for detailed correlation analysis).

\begin{figure}
\centering
\begin{minipage}{1.0\textwidth}
\subfloat[Dnmt3a]{\includegraphics[scale=0.19]{{../meth/jointfigures_best/dnmt3a}.png}}
\hfill
\subfloat[Src]{\includegraphics[scale=0.19]{{../meth/jointfigures_best/src}.png}}
\hfill
\subfloat[Lox]{\includegraphics[scale=0.19]{{../meth/jointfigures_best/lox}.png}}
\end{minipage}
...
\begin{figure}
\begin{minipage}{1.0\textwidth}
\subfloat[xxx]{\includegraphics[scale=0.12]{{../newdata/PDGFRA_15_all}.png}}
\hfill
\subfloat[yyy]{\includegraphics[scale=0.12]{{../newdata/splineplots15/Eln_15_all}.png}}
\hfill
\subfloat[zzz]{\includegraphics[scale=0.12]{{../newdata/splineplots15/INMT_15_all}.png}}
\hfill
\end{minipage}
\caption{Expression profiles over several genes a) xxx, b) yyy, c) zzz}
\label{fig:sup3}
\end{figure}
