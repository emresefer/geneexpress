...
metabolic process, etc~(See Supplementary Table~\ref{tab:supxx}).
...
%These points look slightly more uniform than
%the points identified over gene-expression dataset. 
...
observed the similarity between both datasets by plotting them jointly as in Figure~\ref{fig:methgene} for DNMT3A, SRC,
...
similarity of the identified points when combined with comparison in
the Figure suggest the possibility of global
set of time points that are important for both genomic and epigenetic experiments.

\begin{figure}
\centering
\begin{minipage}{1.0\textwidth}
\subfloat[DNMT3A]{\includegraphics[scale=0.19]{{plots/meth/jointfigures_best/dnmt3a}.png}}
\hfill
\subfloat[SRC]{\includegraphics[scale=0.19]{{plots/meth/jointfigures_best/src}.png}}
\hfill
\subfloat[LOX]{\includegraphics[scale=0.19]{{plots/meth/jointfigures_best/lox}.png}}
\end{minipage}
