...
than mRNA data as miRNA is noisier than mRNA data. After mapping each miRNA to
corresponding genes by TargetScan~\cite{agarwal2015}, we run gene-enrichment analysis
by FuncAssociate~\cite{berriz2003}. We find clusters to be enriched
for several Gene Ontology biological processes~\cite{go}. For
instance, cluster $4$ is enriched for single-organism cellular process, positive regulation of biological
process, regulation of metabolic process, etc~(See Supplementary Results for details).
...
\subsection{Identified Time Points Over Temporal Methylation Data}

We have identified time points over methylation data. We checked
whether these points agree with previous ones.

%Using optimal $8$ points identified over mRNA dataset leads to lower
%error for methylation data.
